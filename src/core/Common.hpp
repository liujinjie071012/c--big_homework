...
struct PhysicsConfig {
    Vec2f gravity = Vec2f(0.0f, 500.0f);     // 重力加速度 (像素/秒^2)
    float timeStep = 0.016f;                  // 物理时间步长
    int substeps = 4;                         // 物理子步数（子步积分可极大提升计算稳定性）
    ...
    float particleElasticity = 0.8f;          // 粒子自碰撞弹性
    float particleFriction = 0.1f;            // 粒子摩擦力

    // SPH 流体仿真配置
    float fluidRadius = 12.0f;                // 流体粒子影响半径 (h)
    float fluidRestDensity = 1000.0f;         // 静止参考密度 (rho_0)
    float fluidStiffness = 2000.0f;           // Tait 状态方程刚度系数 (k)
    float fluidViscosity = 250.0f;            // 粘性系数 (mu)
